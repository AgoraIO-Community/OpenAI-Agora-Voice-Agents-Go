...
4. Implement `server/agent.go` with `agentService` that reads env once, constructs `agentkit.NewAgoraClient`, generates one-hour RTC+RTM ConvoAI tokens, adds `vendors.NewOpenAIGPTLive` with `WithMllm`, starts sessions, stores sessions by `agent_id`, and stops only through the retained session.
...
- `agentService.sessions` is process-local; production multi-instance deployments need shared lifecycle state or request affinity so start and stop reach the same process.
