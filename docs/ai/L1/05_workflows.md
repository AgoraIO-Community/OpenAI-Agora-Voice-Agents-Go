...
- **Greeting, instructions, and context:** change `defaultGreeting` / `defaultInstructions`, or set `AGENT_GREETING` / `AGENT_INSTRUCTIONS` / `AGENT_PRIOR_MESSAGES`. Prior messages are separate from persistent instructions.
- **MLLM:** change `vendors.NewOpenAIGPTLive(...)` while preserving the standard client and MLLM-only shape.
