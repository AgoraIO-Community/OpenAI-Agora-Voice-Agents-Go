...
- `agentService` holds the standard client returned by `agentkit.NewAgoraClient`, built with `option.AreaUS` plus app ID / app certificate credentials. The client detects OpenAI GPT Live as preview and routes start accordingly.
...
- `start(...)` constructs `agentkit.NewAgent(...)`, adds `vendors.NewOpenAIGPTLive` with `WithMllm`, sets server-side RTM/error parameters, then calls `agent.CreateSession(...)` + `session.Start(ctx)`.
- `stop(agentId)` removes the retained process-local session and calls `session.Stop`; unknown IDs are idempotent success and do not trigger a standalone stop request.

## Preview Agent Defaults (server/agent.go)

| Stage | Vendor | Config highlights |
| ----- | ------ | ----------------- |
| MLLM | `NewOpenAIGPTLive` | `vendor: "openai_gpt_live"`, `/v1/live/sessions`, `greeting_message` |

Agent parameters: `DataChannel: "rtm"`, `EnableErrorMessage: true`, `EnableMetrics: true`. Advanced features: `EnableRtm: true`, `EnableTools: false`. Session options: `IdleTimeout: 30`, `ExpiresIn: ExpiresInHours(1)`, `EnableStringUID: false`.
